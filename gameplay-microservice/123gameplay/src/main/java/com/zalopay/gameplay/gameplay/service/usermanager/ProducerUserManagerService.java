...
//    private static final Logger logger = LoggerFactory.getLogger(ProducerGameService.class);
    ...
    @Autowired
    private KafkaTemplate<String, GameResult> kafkaTemplate;


    public void sendMessage(GameResult gameResult) throws JsonProcessingException {
//        logger.info(String.format("#### -> Producing message user manager -> %s", gamePlay.toString()));

        ObjectMapper objectMapper = new ObjectMapper();
        Message<String> message = MessageBuilder.withPayload(objectMapper.writeValueAsString(gameResult))
